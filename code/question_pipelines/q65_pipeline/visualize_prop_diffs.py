...
top_scored_df = pd.read_csv("../../../output/question_pipeline_output/q65_predictions/q65_predictions_filtered.csv")

q65_label = 'Q65_1'  # Label to analyze

# Filter to rows matching the Q65 label
df_q65_label = top_scored_df[top_scored_df['predicted_combined_label'] == q65_label]

# Count sentences per country-year for Q65_label
agg_scored = df_q65_label.groupby(['B_COUNTRY_ALPHA', 'A_YEAR']).agg(
    sentence_count=('predicted_combined_label', 'count')
).reset_index()

# Count total Q65 sentences per country-year (any Q65_*)
total_q65_per_country_year = top_scored_df[
    top_scored_df['predicted_combined_label'].str.startswith('Q65_')
].groupby(['B_COUNTRY_ALPHA', 'A_YEAR']).size().reset_index(name='total_q65_count')

# Merge counts and compute proportion scored per country-year
agg_scored = agg_scored.merge(total_q65_per_country_year, on=['B_COUNTRY_ALPHA', 'A_YEAR'])
agg_scored['proportion_scored'] = agg_scored['sentence_count'] / agg_scored['total_q65_count']
...
# Filter WVS Q65 responses by valid country-year pairs in scored data
valid_country_years = set(zip(agg_scored['B_COUNTRY_ALPHA'], agg_scored['A_YEAR']))

wvs_q65 = wvs_df[['B_COUNTRY_ALPHA', 'A_YEAR', 'Q65', 'S018']].dropna(subset=['B_COUNTRY_ALPHA', 'A_YEAR', 'Q65', 'S018'])
wvs_q65 = wvs_q65[wvs_q65.apply(lambda row: (row['B_COUNTRY_ALPHA'], row['A_YEAR']) in valid_country_years, axis=1)]
wvs_q65['Q65'] = pd.to_numeric(wvs_q65['Q65'], errors='coerce')
wvs_q65 = wvs_q65.dropna(subset=['Q65'])

# Compute weighted proportion per country-year for Q65 = target label
target_label = int(q65_label[-1])

def weighted_proportion(group):
    """
    Calculate the weighted proportion of rows in a group matching the target label.

    The proportion is computed as the weighted count of observations where `Q65`
...
fig.add_trace(go.Scatter(
    ...
    mode='markers',
    name=f'Scored Sentences {q65_label}',
    marker=dict(color='red', size=8),
    hovertemplate='%{x}<br>Sentence Proportion: %{y:.2f}<extra></extra>'
))

fig.update_layout(
    title=f'WVS Weighted vs Scored Sentences Proportion of {q65_label} by Country-Year',
