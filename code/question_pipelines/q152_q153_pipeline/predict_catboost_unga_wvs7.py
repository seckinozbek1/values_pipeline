...
train_df = pd.read_csv("../../../data/labeled_data/Q152_mmr_selected_labeled_combined.csv")
...
# Semantic similarity filtering per predicted label (Used only for scoring with 0 threshold)
def semantic_filter_per_label(sent_emb, keyphrase_embs, keyphrase_list, threshold=0.0):
    ...
    Returns:
        tuple[bool, float, str|None]:
            (True if max similarity ≥ threshold, maximum similarity score,
            and the matched keyphrase if a match exists, else None).
    """
...
unga_df_filtered['perc_above_chance'] = ((unga_df_filtered['prediction_confidence'] - 0.25) / 0.75 * 100).clip(lower=0)
...
# Save results
output_path = "../../../output/question_pipeline_output/q152_q153_predictions/q152_q153_predictions_filtered.csv"
