...
def synonym_augment(sentence, num_replacements=1, seed=None):
    """
    Replace up to `num_replacements` tokens in `sentence` with random WordNet synonyms.

    Args:
        sentence (str): Input text (whitespace-tokenized).
        num_replacements (int): Max tokens to replace. Default 1.
        seed (int|None): RNG seed for reproducibility.

    Returns:
        str: Augmented sentence (or original if no eligible synonyms).

    Notes:
        - Uses NLTK WordNet; only single-word alphabetic lemmas are considered.
        - If no candidates or synonyms exist, returns `sentence` unchanged.
    """
    if seed is not None:
        random.seed(seed)
...
label_encoder.classes_ = np.array(['Q8_2', 'Q8_1'])
...
keep_indices = max_similarities < 0.9
...
train_logloss_at_best = eval_results['learn']['Logloss'][best_iter] if 'Logloss' in eval_results['learn'] else None
val_logloss_at_best = eval_results['validation']['Logloss'][best_iter] if 'Logloss' in eval_results['validation'] else None

print(f"Training Logloss at Best Iteration ({best_iter}): {train_logloss_at_best:.4f}")
print(f"Validation Logloss at Best Iteration ({best_iter}): {val_logloss_at_best:.4f}")
...
probs = model.predict_proba(filtered_val_embeddings)[:, 1]
threshold = 0.5
...
for i in range(n_permutations):
    ...
    perm_probs = perm_model.predict_proba(filtered_val_embeddings)[:, 1]
