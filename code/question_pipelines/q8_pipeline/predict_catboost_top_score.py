...
train_df = pd.read_csv("../../../data/labeled_data/Q8_mmr_selected_labeled_combined.csv")
...
# Take probability of positive class (Q8_1), which is encoded as 1
positive_class_probs = pred_probs_all[:, label2int['Q8_1']]  
threshold = 0.5
...
def semantic_filter_per_label(sent_emb, keyphrase_embs, threshold=0.0):
    ...
    returning a match flag and the similarity score.
    ...
    Returns:
        tuple[bool, float]: (True if max similarity ≥ threshold, max similarity score).
    """
...
def joint_score_filter(
    ...
    Args:
        df (pandas.DataFrame): Input DataFrame containing confidence and similarity columns.
        confidence_col (str): Name of the confidence column (default 'perc_diff_chance_abs').
    ...
    Returns:
        pandas.DataFrame: Copy of `df` containing only rows meeting all three thresholds.
    """
...
# Save results
output_path = "../../../output/question_pipeline_output/q8_predictions/q8_predictions_top_score_filtered.csv"
