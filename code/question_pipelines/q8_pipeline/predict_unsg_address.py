...
unsg_address_df = pd.read_csv("../../../output/master_code_prep_output/unsg_address_corpus.csv")

if 'sentence_text' in unsg_address_df.columns:
    unsg_address_df = unsg_address_df.rename(columns={'sentence_text': 'sentence'})
...
unseen_sentences = unsg_address_df['sentence'].tolist()
batch_size = 32
all_embeddings = []
...
# Semantic similarity filtering per predicted label (Used only for scoring with 0 threshold, could be adjusted)
def semantic_filter_per_label(sent_emb, keyphrase_embs, keyphrase_list, threshold=0.0):
    ...
    returning a match flag, the similarity score, and the matched keyphrase string.
...
unsg_address_df['predicted_combined_label'] = pred_labels
unsg_address_df['prediction_confidence'] = pred_confidences
unsg_address_df['perc_above_chance'] = ((unsg_address_df['prediction_confidence'] - 0.25) / 0.75 * 100).clip(lower=0)
unsg_address_df['semantic_keyphrase_similarity'] = similarities
unsg_address_df['matched_keyphrase'] = matched_keyphrases
...
def joint_score_filter(
    ...
    """
    Filter rows of a DataFrame based on a weighted joint score combining confidence and similarity.

    The joint score for each row is computed as:
    ...
    Args:
        ...
        confidence_weight (float): Weight applied to normalized confidence (default 0.5).
        similarity_weight (float): Weight applied to similarity (default 0.5).
        confidence_threshold (float): Minimum raw confidence value required (default 0.01).
        similarity_threshold (float): Minimum similarity value required (default 0.4).
        joint_threshold (float): Minimum joint score required (default 0.01).

    Returns:
        pandas.DataFrame: Copy of `df` containing only rows meeting all thresholds.
    """
...
final_df = joint_score_filter(unsg_address_df)
...
# Save results
output_path = "../../../output/question_pipeline_output/q8_predictions/q8_predictions_unsg.csv"
