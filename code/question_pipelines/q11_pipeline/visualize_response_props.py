...
# Filter rows where Q11 is not missing and country is in top_scored_sentences
wvs_q11 = wvs_df[['B_COUNTRY_ALPHA', 'Q11']].dropna()
wvs_q11 = wvs_q11[wvs_q11['B_COUNTRY_ALPHA'].isin(valid_countries)]

# Convert Q11 to numeric if not already (e.g., if stored as string)
wvs_q11['Q11'] = pd.to_numeric(wvs_q11['Q11'], errors='coerce')

# Drop rows with non-numeric Q11
wvs_q11 = wvs_q11.dropna(subset=['Q11'])

# Compute per country:
agg = wvs_q11.groupby('B_COUNTRY_ALPHA').agg(
    total_responses=('Q11', 'count'),
    count_ones=('Q11', lambda x: (x == 1).sum())
).reset_index()

# Calculate proportion of Q11=1 responses
...
# Plot proportion of Q11=1 per country
fig = px.scatter(
    agg,
    x='B_COUNTRY_ALPHA',
    y='proportion_ones',
    ...
    labels={
        ...
        'proportion_ones': 'Proportion of Q11=1',
        'total_responses': 'Total Responses'
    },
    title='Proportion of Q11 Responses Equal to 1 by Country',
    size_max=40,
    height=600
)
