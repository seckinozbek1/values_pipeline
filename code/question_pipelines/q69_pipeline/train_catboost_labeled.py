...
def synonym_augment(sentence, num_replacements=1, seed=None):
    ...
    for idx in range(len(replace_indices)):
        idx_word = replace_indices[idx]
        synonyms = wordnet.synsets(words[idx_word])
        lemmas = set(chain.from_iterable([word.lemma_names() for word in synonyms]))
        lemmas.discard(words[idx_word])
        lemmas = [l for l in lemmas if '_' not in l and l.isalpha() and len(l) > 2]
        if lemmas:
            new_word = random.choice(lemmas)
            new_words[idx_word] = new_word
