...
# Filter rows where Q69 is not missing and country is in top_scored_sentences
wvs_q69 = wvs_df[['B_COUNTRY_ALPHA', 'Q69']].dropna()
wvs_q69 = wvs_q69[wvs_q69['B_COUNTRY_ALPHA'].isin(valid_countries)]

# Convert Q69 to numeric if not already (e.g., if stored as string)
wvs_q69['Q69'] = pd.to_numeric(wvs_q69['Q69'], errors='coerce')

# Drop rows with non-numeric Q69
wvs_q69 = wvs_q69.dropna(subset=['Q69'])

# Compute per country:
agg = wvs_q69.groupby('B_COUNTRY_ALPHA').agg(
    total_responses=('Q69', 'count'),
    count_ones=('Q69', lambda x: (x == 1).sum())
).reset_index()

# Calculate proportion of Q69=1 responses
...
# Plot proportion of Q69=1 per country
fig = px.scatter(
    agg,
    x='B_COUNTRY_ALPHA',
    y='proportion_ones',
    ...
    labels={
        ...
        'proportion_ones': 'Proportion of Q69=1',
        'total_responses': 'Total Responses'
    },
    title='Proportion of Q69 Responses Equal to 1 by Country',
    size_max=40,
    height=600
)
