...
top_scored_df = pd.read_csv("../../../output/question_pipeline_output/q70_predictions/q70_predictions_filtered.csv")

q70_label = 'Q70_1'  # Label to analyze

# Filter to rows matching the Q70 label
df_q70_label = top_scored_df[top_scored_df['predicted_combined_label'] == q70_label]

# Count sentences per country-year for Q70_label
agg_scored = df_q70_label.groupby(['B_COUNTRY_ALPHA', 'A_YEAR']).agg(
    sentence_count=('predicted_combined_label', 'count')
).reset_index()

# Count total Q70 sentences per country-year (any Q70_*)
total_q70_per_country_year = top_scored_df[
    top_scored_df['predicted_combined_label'].str.startswith('Q70_')
].groupby(['B_COUNTRY_ALPHA', 'A_YEAR']).size().reset_index(name='total_q70_count')

# Merge counts and compute proportion scored per country-year
agg_scored = agg_scored.merge(total_q70_per_country_year, on=['B_COUNTRY_ALPHA', 'A_YEAR'])
agg_scored['proportion_scored'] = agg_scored['sentence_count'] / agg_scored['total_q70_count']
...
# Filter WVS Q70 responses by valid country-year pairs in scored data
valid_country_years = set(zip(agg_scored['B_COUNTRY_ALPHA'], agg_scored['A_YEAR']))

wvs_q70 = wvs_df[['B_COUNTRY_ALPHA', 'A_YEAR', 'Q70', 'S018']].dropna(subset=['B_COUNTRY_ALPHA', 'A_YEAR', 'Q70', 'S018'])
wvs_q70 = wvs_q70[wvs_q70.apply(lambda row: (row['B_COUNTRY_ALPHA'], row['A_YEAR']) in valid_country_years, axis=1)]
wvs_q70['Q70'] = pd.to_numeric(wvs_q70['Q70'], errors='coerce')
wvs_q70 = wvs_q70.dropna(subset=['Q70'])

# Compute weighted proportion per country-year for Q70 = target label
target_label = int(q70_label[-1])

def weighted_proportion(group):
    """
    Calculate the weighted proportion of rows in a group matching the target label.

    The proportion is computed as the weighted count of observations where `Q70`
...
fig.add_trace(go.Scatter(
    ...
    mode='markers',
    name=f'Scored Sentences {q70_label}',
    marker=dict(color='red', size=8),
    hovertemplate='%{x}<br>Sentence Proportion: %{y:.2f}<extra></extra>'
))

fig.update_layout(
    title=f'WVS Weighted vs Scored Sentences Proportion of {q70_label} by Country-Year',
